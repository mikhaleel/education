...
if($invtype == 'School Fees' OR $invtype == 'Application fee')
{
  $level = $app_infor["level"];
}
else{
  $level = "ALL";
}
...
$payment_type->execute([$invtype,$level, $school_activesession, 1]);
$payment_schedule= $payment_type->fetch();

//$payment_schedule = fetch_payment($pdo, $invtype, $level, $school_activesession);
...
$payment_status->execute([@$appno, $invtype]);
...
if($payment_status->rowCount() == 0)
{
  try {
    ...
    $insert_status->execute([$appno,@$app_infor["programme"],$invtype,$school_activesession,"1",$app_infor["level"],strtoupper(@$app_infor["names"]),$amount,"unpaid",'','','','','']);
    $id = $pdo->lastInsertId();
    
    if($insert_status)
    {
     // echo '<div class="alert alert-success"> Processing payment.....</div><script>setTimeout(function(){location.href="app_transaction?applicant='.encryptor("encrypt", $appno).'&id='.encryptor("encrypt",$id).'&t_f='.encryptor("encrypt",$invtype).'"},1000000000)</script>';
    }
...
else
{
 ...
 // echo '<div class="alert alert-success"> Processing payment.....</div><script>setTimeout(function(){location.href="app_transaction?applicant='.encryptor("encrypt", $appno).'&id='.encryptor("encrypt",$fetchpay['id']).'&t_f='.encryptor("encrypt",$invtype).'"},1000000000)</script>';
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title><?php echo $app_infor["names"];?> Application - Invoice</title>
    <!-- plugins:css -->
                  ...
                  <div class="card-body">
                    ...
                    <div class="container-fluid d-flex justify-content-between">
                      ...
                      <div class="col-lg-3 pr-0">
                        ...
                        <p class="text-right"><?php echo $app_infor["names"];?>, <br> <?php echo $app_infor["application_no"];?>, <br> <?php echo $app_infor["email"];?>.</p>
                      </div>
                    </div>
                        ...
                        <table class="table">
                          ...
                          <tbody>
                            <tr class="text-right">
                              ...
                              <td colspan="3" class="text-left"><?php echo @strtoupper($invtype).' FOR '.$level.' - '.$app_infor["programme"]. ' ('.$school_activesession.')';?></td>
                    ...
                    {
                    $appno =@encryptor("decrypt",$_GET['applicant']);
                    $id= @encryptor("decrypt",$_GET['id']);
                    $req = $pdo->prepare("SELECT * FROM `applicant` where `application_no`='".$appno."'");
                    $req->execute();
                    $row= $req->fetch();
                    ...
                    try {
                    $payment_status = $pdo->query("SELECT * FROM `stu_payloader` WHERE `id`='$id' and `matno` = '$appno'");
                    ...
                    ?>
                      ...
                      <form  id="paymentForm">
                          ...
                          <input name="student_id" type="hidden" required="required" id="student_id" value="<?php echo encryptor("encrypt",$appno);?>">
                          <input name="matno" type="hidden" required="required" id="matno" value="<?php echo $appno;?>">		 
                    ...
                    { // get the table field name -- 
                      ...
                      ?>
                      <a href="?payinv=1&<?php echo encryptor("encrypt", $appno).'&invtyp='.encryptor("encrypt",$invtype).'&appno='.encryptor("encrypt", $appno).'&applicant='.encryptor("encrypt", $appno).'&id='.encryptor("encrypt",$id).'&t_f='.encryptor("encrypt",$invtype);?>" class="btn btn-primary float-end mt-4 ms-2">
