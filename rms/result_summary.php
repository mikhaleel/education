...
$dept_id = $_GET["department"]; $programmehead = 'FULL TIME PROGRAMME';
...
<div id="printdivcontent">
    ...
    <?php echo $programmehead;?><br>
    <?php //echo $conf_session;?><br>
    RESULT SUMMARY
    </h5>
    <hr>
          <?php  //$prog_rows = $prgquery->fetch();
            ...
            //$prgquery = $pdo->query("SELECT * FROM `programmes` WHERE `dept_id`= '$dept_id' GROUP BY `programme`");
            // while($prog_rows = $prgquery->fetch())
            // {
            $programme = $_GET["department"];
            $fountprog = $_GET["department"];
