...
$hid = encryptor("decrypt",$_GET["hid"]);
$enchid = $_GET["hid"];
...
$invtype = "Accommodation";
$school_activesession;
$school_activesemester;
$st_progrm = $stu_infor["programme"];
$payment_type = $pdo->prepare("SELECT * FROM `hostels` WHERE  `id`=?");
$payment_type->execute([$hid]);
...
if($payment_status->rowCount() == 0)
{
  try {
    ...
    $insert_status->execute([$matno,@$st_progrm,$invtype,$school_activesession,$school_activesemester,$stu_infor["level"],strtoupper(@$stu_infor["names"]),$amount,"unpaid",$hid,'','','','']);
    $id = $pdo->lastInsertId();
    if($insert_status){}
                ...
                <div class="card px-2">
                  <div class="card-body">
                        ...
                        <table class="table">
                          ...
                          <tbody>
                            <tr class="text-right">
                              ...
                              <td>N<?php echo $amount = $payment_schedule['amount'];?></td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                    ...
                    { // get the table field name -- 
                      ...
                      ?>
                      <a href="?payinv=1&<?php echo encryptor("encrypt", $matno).'&invtyp='.encryptor("encrypt",$invtype).'&matno='.encryptor("encrypt", $matno).'&students='.encryptor("encrypt", $matno).'&id='.encryptor("encrypt",$id).'&t_f='.encryptor("encrypt",$invtype);?>&hid=<?php echo $enchid;?>" class="btn btn-primary float-end mt-4 ms-2">
